...
class WifiReminderTrigger(val context: Context) {
    ...
    fun connected(web: String){
        ...
        val reminders = dao.getActive(web, ReminderWhen.IS)
        dao.close()
        notifyReminders(reminders)
    }
    fun desconnected(web: String){
        val dao = ReminderDAO(context)
        val reminders = dao.getActive(web, ReminderWhen.IS)
        dao.close()
        notifyReminders(reminders)
    ...
    private fun notifyReminders(reminders: List<Reminder>) {
        ...
        reminders.forEach {
            it.active = false
            dao.alt(it)
            val connect = if(it.rWhen == ReminderWhen.IS) context.getString(R.string.contected_to) else context.getString(R.string.descontected_to)
